...
pub struct Donations {
    qr_tondi_ng_fund : HashMap<String, (String,load::Bytes)>,
}

impl Donations {

    pub const ADDRESS_TONDI_NG_FUND: &'static str = "tondi:qq2efzv0j7vt9gz9gfq44e6ggemjvvcuewhzqpm4ekf4fs5smruvs3c8ur9rp";

    pub fn new(_runtime: Runtime) -> Self {
        Self { 
            qr_tondi_ng_fund : Default::default(),
        }
    }

    fn qr_tondi_ng_fund(&mut self) -> (String,load::Bytes) {

        let (uri,qr) = match self.qr_tondi_ng_fund.entry(theme_color().name.clone()) {
            Entry::Occupied(entry) => entry.into_mut(),
            Entry::Vacant(entry) => {
                let uri = format!("bytes://{}-{}.svg", Self::ADDRESS_TONDI_NG_FUND, theme_color().name);
                let qr = render_qrcode(Self::ADDRESS_TONDI_NG_FUND, 128, 128);
    ...
    fn render_destination(&self, ui: &mut Ui, (uri, qr) : (String,load::Bytes) ) {
        ...
        let response = ui.add(Label::new(format!("{} {CLIPBOARD_TEXT}", format_address_string(Self::ADDRESS_TONDI_NG_FUND, Some(12)))).sense(Sense::click()))
        ...
        if response.clicked() {
            //ui.output_mut(|o| Self::ADDRESS_TONDI_NG_FUND.clone_into(&mut o.copied_text));
            ui.ctx().copy_text(Self::ADDRESS_TONDI_NG_FUND.to_string());
    ...
    fn render(
        ...
        Panel::new(self)
            ...
            .with_body(|this,ui| {
                ...
                let tondi_ng_fund = this.qr_tondi_ng_fund();
                this.render_destination(ui, tondi_ng_fund);
                ui.label(" ");
