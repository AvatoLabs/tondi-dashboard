...
// https://api.coingecko.com/api/v3/simple/price?ids=tondi&vs_currencies=usd%2Ccny&include_market_cap=true&include_24hr_vol=true&include_24hr_change=true
// {
//     "tondi": {
//       "usd": 0.137395,
...
struct CoinGeckoSimplePrice {
    tondi: Option<AHashMap<String, f64>>,
}
...
impl From<CoinGeckoSimplePrice> for MarketDataMap {
    fn from(data: CoinGeckoSimplePrice) -> Self {
        ...
        if let Some(tondi) = data.tondi {
            prices = group_by_currency_prefix(&tondi);
        }
        prices
    }
}
