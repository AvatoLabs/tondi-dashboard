...
impl Storage {
    ...
    pub fn storage_root(&self) -> PathBuf {
        self.storage_root
            ...
            .unwrap()
            .clone()
            .or_else(|| Some(tondid_lib::daemon::get_app_dir()))
            .unwrap()
    }
    ...
    pub fn update(&self, options: Option<StorageUpdateOptions>) {
        ...
        let rusty_tondi_app_dir = self.storage_root();
        if !rusty_tondi_app_dir.exists() {
            return;
        }
        ...
        spawn(async move {
            ...
            let paths = std::fs::read_dir(rusty_tondi_app_dir).unwrap();
            for path in paths {
                let path = path?.path();
    ...
    fn update_folder_size(&self, network: Network, folder_size: u64, path: PathBuf) {
        use tondi_metrics_core::data::as_data_size;
                        ...
                        for folder in folders.iter_mut() {
                            ...
                            .show(ui, |ui| {
                                ...
                                if *confirm_deletion {
                                    ...
                                    ui.label(i18n("This action will erase Tondi database and logs"));
                                    ui.label("");
